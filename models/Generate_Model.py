...
from models import models_vit, prompt_blocks
import torch.nn.functional as F
from typing import Any, Callable, Dict, Optional, Sequence, Set, Tuple, Type, Union, List
from models import fusion_gate
...
class GenerateModel(nn.Module):
    def __init__(self, args):
        ...
        self.generate_projector = nn.ModuleList([prompt_blocks.SSMBlock(hidden_dim=768, d_state=16, d_conv=4) for _ in range(12)])
        
        
       
        self.fusion_model = fusion_gate.MSC2F()
    ...
    def _build_audio_model(self, model_name='vit_base_patch16', drop_path_rate=0.1, global_pool=False, mask_2d=True, use_custom_patch=False, ckpt_path='/mnt/audiomae_pretrained.pth'):
        ...
        emb =  torch.randn(1, self.n_audio + self.n_progr*self.prompt_depth + 1, 768)
    ...
    def _build_image_model(self, model_name='vit_base_patch16', ckpt_path='/mnt/mae_face_pretrain_vit_base.pth', 
        ...
        pos_embed = torch.randn(1, self.image_encoder.pos_embed.size(1) + (len(self.image_encoder.blocks))*self.n_progr, 768)  
        pos_embed[:,:-(len(self.image_encoder.blocks))*self.n_progr,:] = self.image_encoder.pos_embed
    ...
    def forward(self, image, audio):
        ...
        assert t == 16
        B = image.shape[0]
        image_generatedPrompt_list = []
        audio_generatedPrompt_list = []
        ...
        for ii in range(len(self.audio_model.blocks)):
            ...
            audio_next_prompt, image_next_prompt = self.generate_projector[ii](image, audio)
            
            audio_generatedPrompt_list.append(audio_next_prompt)
            image_generatedPrompt_list.append(image_next_prompt)
            
            image = self.image_encoder.forward_block_post(ii, image, audio_generatedPrompt_list[ii], image_generatedPrompt_list[ii], B)
            audio = self.audio_model.forward_block_post(ii, audio, image_generatedPrompt_list[ii], audio_generatedPrompt_list[ii])

        # (n, t, -1, D)
        image = image.view(n, 16, -1, image.size(-1))
        

        output, adv_loss, mmd_loss = self.fusion_model(image, audio)
        
        return output, adv_loss, mmd_loss
