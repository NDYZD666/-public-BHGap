...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str,default='/root//BHGap/checkpoints/model.pth')
...
def main(set, args):
    ...
    uar, war = computer_uar_war(val_loader, model, args.checkpoint, data_set, args)
...
def computer_uar_war(val_loader, model, checkpoint_path, data_set, args):
    ...
    avg_mmd_loss = total_mmd_loss / total_samples
    ...
    print("UAR: %0.2f" % uar)
    print("WAR: %0.2f" % war)
    print("Average Adversarial Loss: %0.4f" % avg_adv_loss)
    print("Average MMD Loss: %0.4f" % avg_mmd_loss)
